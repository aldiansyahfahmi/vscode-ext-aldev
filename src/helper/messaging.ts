...
let criticalErrorOptions: vscode.MessageItem[] = [{ title: ErrorOptionType.report }, { title: ErrorOptionType.ignore }];

let retryableErrorOptions: vscode.MessageItem[] = [{ title: ErrorOptionType.tryAgain }, { title: ErrorOptionType.close }];
...
export async function showRetryableError(error: Error): Promise<boolean> {
  const response: vscode.MessageItem | undefined = await vscode.window.showWarningMessage(`An error has occurred:\n${error.message}`, {}, ...retryableErrorOptions);
