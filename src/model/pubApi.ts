...
import { getRestApiError, PackageSearchInfo, PageSearchInfo, PubApiSearchError } from "./pubError";
...
export class PubAPI {
  ...
  private async getPageJson(id: number = 1): Promise<any> {
    const res: rm.IRestResponse<any> = await this.restClient.get(this.generateUrl(`packages?page=${id}`));
    if (!res.result) {
  ...
  public async searchPackage(query: string): Promise<PubResponse<PubPackageSearch>> {
    const fullQuery = `search?q=${escapeHtml(query)}`;
    try {
      const res: rm.IRestResponse<any> = await this.restClient.get(this.generateUrl(fullQuery));
      if (!res.result) {
  ...
  public async smartSearchPackage(query: string, singleReturnThreshold: number = 0.1): Promise<PubResponse<PubPackageSearch>> {
    const fuseOptions = {
      shouldSort: true,
      includeScore: true,
      threshold: 0.5,
    ...
    const response: PubResponse<PubPackageSearch> = await this.searchPackage(query);
    if (!response.result) {
      throw new PubApiSearchError(PackageSearchInfo(query));
    }
    ...
    const rankedResult = (fuse.search(query) as any[]).filter((element) => !element.item.package.startsWith("dart:"));

    const significantResults = rankedResult.filter((element) => element.score <= singleReturnThreshold);
    if (getSettings().autoAddPackage && significantResults.length > 0 && (significantResults.length === 1 || significantResults[0].score === 0)) {
      return {
        status: ResponseStatus.SUCCESS,
      ...
      return {
        status: ResponseStatus.SUCCESS,
        result: new PubPackageSearch(rankedResult.map((element) => element.item.package)),
      };
    }
  }
  ...
  public async getPackage(name: string): Promise<PubResponse<PubPackage>> {
    try {
      const res: rm.IRestResponse<Object> = await this.restClient.get(this.generateUrl(`packages/${name}`));
      if (!res.result) {
